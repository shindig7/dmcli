...
class Generator:
    ...
    def generate(self, chart_id: str) -> Result[dict[str, Any]]:
        try:
            assert getenv("CHARTOPIA_USER") is not None and getenv("CHARTOPIA_PASS") is not None
        except AssertionError:
            return Err("Please set your CHARTOPIA_USER and CHARTOPIA_PASS environment variables")
        auth = HTTPBasicAuth(getenv("CHARTOPIA_USER"), getenv("CHARTOPIA_PASS"))
